...
public class HtmlGetter {
	 ...
	 * @param url
	 *            指定URL
	 * @param includeOtherSite
	 *            查找返回内容中的URL是否包括本站以外的
	 * @param withInit
	 *            构造时初始化，获取页面内容，获取图片路径，获取页面包含的url路径
	 * @throws Exception
	 */
	public HtmlGetter(String url, boolean includeOtherSite, boolean withInit)
			throws Exception {
	...
	public void initHtml() throws Exception {
		...
		Pattern pat = Pattern
				.compile("(?<=(src=\"|url\\())\\S*\\.(jpg|gif|png|bmp|ico)");
		...
		while (m.find()) {
			String temp = m.group(0);
			if (!temp
					.matches("http:\\S*?\\.(jpg|bmp|js|gif|png|ico|pdf|exe|jar|doc|cxl|css)")) {
	...
	public void saveAllPhotos(HtmlGetter hg) throws Exception {
		...
		if (!this.totalUrls.containsAll(hg.htmlURLs)) {
			...
			for (String page : hg.htmlURLs) {
				HtmlGetter hgTemp = new HtmlGetter(page, hg.includeOtherSite,
						true);
	...
	private void save(String photo, String savePath) {
		String fileName = photo.substring(photo.lastIndexOf("/") < 0 ? 0
				: photo.lastIndexOf("/"), photo.length());
		InputStream is = null;
	...
	public static void main(String[] args) throws Exception {
		...
		HtmlGetter hg = new HtmlGetter("http://www.baidu.com/", true, true);
		hg.setPhotoSavePath("D:/images");
		hg.saveAllPhotos(hg);
