package com.app.spider.httpclient;
...
public class SpiderHtml {
	...
	private static final String forwardURL = baseDir+"dispatcher.php/module/Personal/?skip_fill=1";
	private static final String hostCss  = baseUrl+"style.txt";
    ...
    private static String  createHtml(HttpClient client, String newUrl) throws  Exception {
		...
		String filePath = baseUrl	+ format.format(new Date() )+ "_" + 1 + ".html";
		...
		try {
			...
			for (Element js : scripts) {
				...
				if (jsrc.contains("/fValidate.one.js")) {
					...
					jsrc = jsrc.replace(oldJS,serverJS);
